      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md">
          <header className="mb-8 text-center">
            <div className="w-14 h-14 mx-auto rounded-2xl bg-gold-100 text-gold-700 grid place-items-center mb-4">
              <Truck size={28} />
            </div>
            ...
            <p className="text-sm text-ink-500 mt-1">
              Sign in to view today's assigned pickups.
            </p>
          </header>
          ...
          >
            ...
            <Field label="Password" htmlFor="col-password">
            ...
            <button
              type="submit"
              disabled={disabled}
              className="w-full inline-flex items-center justify-center gap-2 h-11 rounded-xl bg-gold-600 hover:bg-gold-700 text-white font-semibold text-sm transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isAuthenticating && <Loader2 size={16} className="animate-spin" />}
              {isAuthenticating ? 'Signing in…' : 'Sign in as Collector'}
            </button>

            <div className="text-center text-xs text-ink-500 space-x-3">
              <Link to="/login" className="hover:text-ink-700">User portal</Link>
              <span aria-hidden>·</span>
              <Link to="/admin/login" className="hover:text-ink-700">Admin sign-in</Link>
            </div>
          </form>

          <p className="mt-6 text-center text-[11px] text-ink-400">
            Need an account? Contact your dispatch coordinator.
          </p>
        </div>
      </div>
    </div>
