      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md">
          <header className="mb-8 text-center">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-ink-800">Create your account</h1>
          ...
          <form
            onSubmit={onSubmit}
            className="bg-white rounded-2xl shadow-sm border border-ink-100 p-6 sm:p-7 space-y-4"
            noValidate
          >
            {error && (
              <Banner tone="error">
            ...
            <Field label="Phone (Nigeria)" htmlFor="reg-phone" hint="e.g. +234 803 555 0142">
              <input
                id="reg-phone"
                type="tel"
                autoComplete="tel"
            ...
            <Field label="Password" htmlFor="reg-password" hint={`At least ${MIN_PASSWORD} characters`}>
            ...
            <label className="flex items-start gap-2.5 select-none">
              <input
                ...
                className="mt-0.5 w-4 h-4 rounded border-ink-300 text-eco-600 focus:ring-eco-500"
              />
              <span className="text-xs text-ink-600">
                I agree to the{' '}
                <Link to="/terms" className="text-eco-700 hover:underline">Terms</Link> and{' '}
                <Link to="/privacy" className="text-eco-700 hover:underline">Privacy Policy</Link>.
              </span>
            </label>
            ...
            <p className="text-center text-sm text-ink-600 pt-1">
              ...
              <Link to="/login" className="text-eco-700 font-semibold hover:underline">
                Sign in
              </Link>
            </p>
          </form>
...
function PasswordStrength({ password }) {
  ...
  const checks = [
    { ok: password.length >= MIN_PASSWORD, label: `${MIN_PASSWORD}+ characters` },
  ...
  return (
    <ul className="mt-2 grid grid-cols-2 gap-y-1 text-[11px] text-ink-500">
      {checks.map((c) => (
        ...
        >
          <Check size={12} className={c.ok ? 'opacity-100' : 'opacity-30'} />
          {c.label}
        </li>
      ))}
    </ul>
